...
dotenv.config();	//.env -> process.env의 하위 속성으로 추가
...
// body-parser - 라우터 최 상단에 위치
app.use(express.json());
app.use(express.urlencoded({ extended : false }));

/*정적 경로*/
// app.use("/", express.static(path.join(__dirname, "public")));		메인경로 "/" 생략 가능
app.use(express.static(path.join(__dirname, "public")));

/* 라우터 등록 */
// app.use("/", indexRouter); 		메인경로 "/" 생략 가능
app.use(indexRouter);
...
app.use((err, req, res, next) => {
	...
	if (process.env.NODE_EMV === "production") {		// 서비스 중-> stack 정보는 제거된다
		delete data.stack;
	}
